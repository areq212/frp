...
func (sv *XUDPVisitor) dispatcher() {
	...
	for {
		...
		// Open a tunnel connection to the target frpc. If there is already a successful
		// hole-punching connection, it will be reused. Otherwise, it will block and wait
		// for a successful hole-punching connection until timeout.
		tunnelConn, err = sv.openTunnel(sv.ctx)
		if err != nil {
			xl.Warnf("open tunnel error: %v, try to reconnect", err)
			continue
		}

		rwc, recycleFn, err := wrapVisitorConn(tunnelConn, sv.cfg.GetBaseConfig())
		if err != nil {
			xl.Errorf("wrap tunnel connection error: %v", err)
			tunnelConn.Close()
			continue
		}

		// tunnelConn always be closed when worker done.
		func() {
			defer recycleFn()
			sv.worker(netpkg.WrapReadWriteCloserToConn(rwc, tunnelConn), firstPacket)
		}()
...
func (sv *XUDPVisitor) worker(workConn net.Conn, firstPacket *msg.UDPPacket) {
	...
	// The peers exchange messages directly, so always use the default codec
	// regardless of each side's wireProtocol setting for the frps transport.
	payloadConn := msg.NewConn(workConn, msg.NewReadWriter(workConn, ""))
	...
	workConnReaderFn := func(payloadConn *msg.Conn) {
		...
		for {
			...
			// frpc will send heartbeat in tunnel connection to frpc visitor for keeping alive
			_ = payloadConn.SetReadDeadline(time.Now().Add(60 * time.Second))
			if rawMsg, errRet = payloadConn.ReadMsg(); errRet != nil {
				xl.Warnf("read from tunnel connection for user udp conn error: %v", errRet)
				return
			}
			...
			case *msg.UDPPacket:
				if errRet := errors.PanicToError(func() {
					sv.readCh <- m
					xl.Tracef("frpc visitor get udp packet from tunnel connection, len: %d", len(m.Content))
				}); errRet != nil {
					xl.Infof("reader goroutine for udp tunnel connection closed")
					return
				}
			}
		}
	...
	workConnSenderFn := func(payloadConn *msg.Conn) {
		...
		if firstPacket != nil {
			if errRet = payloadConn.WriteMsg(firstPacket); errRet != nil {
				xl.Warnf("sender goroutine for udp tunnel connection closed: %v", errRet)
				return
			}
			xl.Tracef("send udp package to tunnel connection, len: %d", len(firstPacket.Content))
		}
		...
		for {
			...
			case udpMsg, ok := <-sv.sendCh:
				if !ok {
					xl.Infof("sender goroutine for udp tunnel connection closed")
					return
				}
				...
				xl.Tracef("send udp package to tunnel connection, len: %d", len(udpMsg.Content))
			case <-closeCh:
				return
			}
		}
...
func (sv *XUDPVisitor) openTunnel(ctx context.Context) (conn net.Conn, err error) {
	...
	for {
		...
		case <-timer.C:
			...
			if err != nil {
				if !goerrors.Is(err, ErrNoTunnelSession) {
